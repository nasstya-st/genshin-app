...
Locals = enum.Enum(
	value='Locals',
	names=(
		"calla_lily cecilia dandelion_seed philanemo_mushroom lamp_grass valberry yellow_lotus rukkhashava blue_lotus "
		"windwheel_aster wolfhook cor_lapis glaze_lily jueyun_chili noctilucous_jade qingxin sea_ganoderma sango_pearl "
		"silk_flower starconch violetgrass amakumo_fruit crystall_marrow dendrobium padisarah sakura_bloom onikabuto "
		"fluorescent_fungus naku_weed henna_berry scarab")
)

Boss = enum.Enum(
	value="Boss",
	names=(
		"geo_cube pyro_cube cryo_cube hydro_cube electro_cube anemo_cube cryo_plant pyro_plant serpent electro_plant "
		"hydro_oceanid geovishap maguu_kenki mech_array electro_oceanid wolflord coral_defenders beak algorithm "
		"dendro_cube drake")
)
...
Boss_tals = enum.Enum(
	value='Boss_tals',
	names=(
		"dvalin_plume dvalin_claw dvalin_sigh boreas_tail boreas_ring boreas_spirit childe_tusk raiden_meaning "
		"childe_shard childe_shadow azhdaha_crown azhdaha_branch azhdaha_scale signora_moment raiden_tears raiden_mudra"
		" signora_butterfly signora_heart scar_strings scar_mirror scar_bell")
)
Enemies_gen = enum.Enum(
	value='Enemies_gen',
	names=(
		"slime hilichurl_mask samachurl hilichurl_arrow fatui treasure_hoarder whopperflower "
		"nobushi spectral fungi_gen eremites")
)

Enemies_elite = enum.Enum(
	value='Enemies_elite',
	names=(
		"hilichurl_horn ley_line chaos_part mist knives bone chaos_sentinel prism fungi_el statuette drake_chaos "
		"riftwolf")
)

Asc_weap = enum.Enum(
	value='Asc_weap',
	names="decarabian boreal gladiator guyun elixir aerosiderite branch narukami mask talisman oasis might"
)
...
# Characters list
chars_names4 = sorted([
	'Sayu', 'Sucrose', 'Chongyun', 'Diona', 'Kaeya', 'Rosaria', 'Beidou', 'Fischl', 'Sara', 'Kuki', 'Lisa', 'Razor',
	'Gorou', 'Ningguang', 'Noelle', 'Yun_Jin', 'Barbara', 'Xingqiu', 'Amber', 'Bennett', 'Thoma', 'Xiangling',
	'Xinyan', 'Yanfei', 'Heizou', 'Collei', 'Dori', 'Candace'])
chars_names5 = sorted([
	'Jean', 'Kazuha', 'Venti', 'Xiao', 'Aloy', 'Eula', 'Ganyu', 'Ayaka', 'Qiqi', 'Shenhe', 'Keqing', 'Raiden',
	'Yae_Miko', 'Albedo', 'Itto', 'Zhongli', 'Ayato', 'Mona', 'Kokomi', 'Tartaglia', 'Yelan', 'Diluc', 'Hu_Tao',
	'Klee', 'Yoimiya', 'Tighnari', 'Nilou', 'Cyno', 'Nahida'])
# ______________________________________________________________________________________________________________________
# Weapons lists
bows = sorted([
	"Skyward_Harp", "Amos'_Bow", "Alley_Hunter", "The_Viridescent_Hunt", "The_Stringless", "Sacrificial_Bow", "Rust",
	"Royal_Bow", "Prototype_Crescent", "Mouun's_Moon", "Mitternachts_Waltz", "Hamayumi", "Favonius_Warbow",
	"Fading_Twilight", "Compound_Bow", "Blackcliff_Warbow", 'Thundering_Pulse'])
catalysts = sorted([
	"Blackcliff_Agate", "The_Widsith", "Solar_Pearl", "Royal_Grimoire", "Sacrificial_Fragments", "Favonius_Codex",
	"Prototype_Amber", "Oathsworn_Eye", "Wine_and_Song", "Mappa_Mare", "Hakushin_Ring", "Frostbearer",
	"Eye_of_Perception", "Dodoco_Tales", 'Lost_Prayer_to_the_Sacred_Winds'])
polearms = sorted([
	"Staff_of_Homa", "Prototype_Starglitter", "Lithic_Spear", "The_Cath", "Crescent_Pike", "Blackcliff_Pole",
	"Favonius_Lance", "Dragonspine_Spear", "Deathmatch", "Dragon's_Bane", "Kitain_Cross_Spear", 'Skyward_Spine',
	"Royal_Spear", "Wavebreaker's_Fin", "Primordial_Jade_Winged-Spear", 'Missive_Windspear'])
claymores = sorted([
	"Redhorn_Stonethresher", "Whiteblind", "The_Bell", "Serpent_Spine", "Snow-Tombed_Starsilver", "Royal_Greatsword",
	"Sacrificial_Greatsword", "Lithic_Blade", "Blackcliff_Slasher", "Rainslasher", "Prototype_Archaic", "Akuoumaru",
	"Luxurious_Sea-Lord", "Katsuragikiri_Nagamasa", "Favonius_Greatsword", "Forest_Regalia"
])
swords = sorted([
	"Amenoma_Kageuchi", "The_Black_Sword", "The_Alley_Flash", "Royal_Longsword", 'Kagotsurube_Isshin',
...
all_chars = {
	'Amber': Character(Gems.pyro, Talents.freedom, Locals.lamp_grass, Enemies_gen.hilichurl_arrow,
	...
	'Candace': Character(Gems.hydro, Talents.admonition, Locals.henna_berry, Enemies_gen.eremites, Boss.algorithm,
					  	 Boss_tals.raiden_tears),
	'Nilou': Character(Gems.hydro, Talents.praxis, Locals.padisarah, Enemies_gen.fungi_gen, Boss.drake,
					   Boss_tals.raiden_tears),
	'Cyno': Character(Gems.electro, Talents.admonition, Locals.scarab, Enemies_gen.samachurl, Boss.electro_plant,
					  Boss_tals.raiden_mudra),
	'Nahida': Character(Gems.dendro, Talents.ingenuity, Locals.blue_lotus, Enemies_gen.fungi_gen, Boss.dendro_cube,
						Boss_tals.scar_strings)
}
# ______________________________________________________________________________________________________________________
# Dict with all weapon and its resources
all_weap = {
	"Skyward_Blade": Weapon(5, Asc_weap.boreal, Enemies_elite.ley_line, Enemies_gen.slime),
	...
	"The_Viridescent_Hunt": Weapon(4, Asc_weap.decarabian, Enemies_elite.hilichurl_horn, Enemies_gen.hilichurl_arrow),
	...
	"Mitternachts_Waltz": Weapon(4, Asc_weap.decarabian, Enemies_elite.hilichurl_horn, Enemies_gen.treasure_hoarder),
	...
	"Lost_Prayer_to_the_Sacred_Winds": Weapon(5, Asc_weap.gladiator, Enemies_elite.chaos_part, Enemies_gen.slime),
	...
	"Sacrificial_Fragments": Weapon(4, Asc_weap.gladiator, Enemies_elite.chaos_part, Enemies_gen.treasure_hoarder),
	...
	'Skyward_Spine': Weapon(5, Asc_weap.gladiator, Enemies_elite.chaos_part, Enemies_gen.samachurl),
	'Thundering_Pulse': Weapon(5, Asc_weap.narukami, Enemies_elite.prism, Enemies_gen.hilichurl_arrow),
	'Missive_Windspear': Weapon(4, Asc_weap.boreal, Enemies_elite.statuette, Enemies_gen.slime)
}
# ______________________________________________________________________________________________________________________
...
resources_clear = {
	'total_exp': 0, 'exp_weapon': 0, 'total_mora': 0, 'crystal_exp': [0, 0, 0, 0], 'book_exp': [0, 0, 0, 0],

	'calla_lily': 0, 'dandelion_seed': 0, 'philanemo_mushroom': 0, 'lamp_grass': 0, 'amakumo_fruit': 0,
	'yellow_lotus': 0, "windwheel_aster": 0, 'wolfhook': 0, 'cor_lapis': 0, 'glaze_lily': 0, 'jueyun_chili': 0,
	'noctilucous_jade': 0, "qingxin": 0, "silk_flower": 0, 'starconch': 0, 'violetgrass': 0, "rukkhashava": 0,
	'crystall_marrow': 0, 'dendrobium': 0, "padisarah": 0, "fluorescent_fungus": 0, 'naku_weed': 0, 'cecilia': 0,
	'sakura_bloom': 0, 'sango_pearl': 0, 'sea_ganoderma': 0, "blue_lotus": 0, 'onikabuto': 0, 'valberry': 0,
	'scarab': 0, 'henna_berry': 0,

	'geo_cube': 0, 'pyro_cube': 0, 'cryo_cube': 0, 'hydro_cube': 0, 'electro_cube': 0, "beak": 0, "serpent": 0,
	'anemo_cube': 0, 'cryo_plant': 0, 'pyro_plant': 0, "hydro_oceanid": 0, 'geovishap': 0, "electro_plant": 0,
	'maguu_kenki': 0, 'mech_array': 0, 'electro_oceanid': 0, 'wolflord': 0, 'coral_defenders': 0, 'drake': 0,
	'dendro_cube': 0, 'algorithm': 0,

	"slime": [0, 0, 0, 0], 'hilichurl_mask': [0, 0, 0, 0], 'samachurl': [0, 0, 0, 0], 'fatui': [0, 0, 0, 0],
	'hilichurl_arrow': [0, 0, 0, 0], 'treasure_hoarder': [0, 0, 0, 0], 'whopperflower': [0, 0, 0, 0],
	"nobushi": [0, 0, 0, 0], 'spectral': [0, 0, 0, 0], 'fungi_gen': [0, 0, 0, 0], "eremites": [0, 0, 0, 0],

	'anemo': [0, 0, 0, 0, 0], 'geo': [0, 0, 0, 0, 0], 'dendro': [0, 0, 0, 0, 0], 'cryo': [0, 0, 0, 0, 0],
	'pyro': [0, 0, 0, 0, 0], 'electro': [0, 0, 0, 0, 0], 'hydro': [0, 0, 0, 0, 0],

	'dvalin_plume': 0, 'dvalin_claw': 0, 'dvalin_sigh': 0, 'boreas_tail': 0, 'boreas_ring': 0, "raiden_meaning": 0,
	'boreas_spirit': 0, "childe_tusk": 0, "childe_shard": 0, 'childe_shadow': 0, 'azhdaha_crown': 0, 'raiden_tears': 0,
	'azhdaha_branch': 0, 'azhdaha_scale': 0, "signora_moment": 0, "signora_butterfly": 0, 'signora_heart': 0,
	'raiden_mudra': 0, 'scar_strings': 0, 'scar_mirror': 0, 'scar_bell': 0,

	'freedom': [0, 0, 0, 0], 'prosperity': [0, 0, 0, 0], 'transience': [0, 0, 0, 0], 'resistance': [0, 0, 0, 0],
	'ballad': [0, 0, 0, 0], 'diligence': [0, 0, 0, 0], 'gold': [0, 0, 0, 0], 'elegance': [0, 0, 0, 0],
	'light': [0, 0, 0, 0], 'admonition': [0, 0, 0, 0], 'ingenuity': [0, 0, 0, 0], 'praxis': [0, 0, 0, 0],

	'decarabian': [0, 0, 0, 0, 0], 'boreal': [0, 0, 0, 0, 0], 'gladiator': [0, 0, 0, 0, 0], 'guyun': [0, 0, 0, 0, 0],
	'elixir': [0, 0, 0, 0, 0], 'aerosiderite': [0, 0, 0, 0, 0], 'branch': [0, 0, 0, 0, 0], 'narukami': [0, 0, 0, 0, 0],
	'mask': [0, 0, 0, 0, 0], 'talisman': [0, 0, 0, 0, 0], 'oasis': [0, 0, 0, 0, 0], 'might': [0, 0, 0, 0, 0],

	'hilichurl_horn': [0, 0, 0, 0], 'ley_line': [0, 0, 0, 0], 'chaos_part': [0, 0, 0, 0], 'mist': [0, 0, 0, 0],
	'knives': [0, 0, 0, 0], 'bone': [0, 0, 0, 0], 'chaos_sentinel': [0, 0, 0, 0], 'prism': [0, 0, 0, 0],
	'fungi_el': [0, 0, 0, 0], 'statuette': [0, 0, 0, 0], 'drake_chaos': [0, 0, 0, 0], 'riftwolf': [0, 0, 0, 0]
}
...
exp_lvl = (
	0, 1000, 1325, 1700, 2150, 2625, 3150, 3725, 4350, 5000, 5700, 6450, 7225, 8050, 8925, 9825, 10750, 11725,
	...
	167425, 171300, 175225, 179175, 183175, 216225, 243025, 273100, 306800, 344600, 386950, 434425, 487625, 547200)
# And for weapon
exp_weap = (
	0, 400, 625, 900, 1200, 1550, 1950, 2350, 2800, 3300, 3800, 4350, 4925, 5525, 6150, 6800, 7500, 8200, 8950,
